...
export default class Parser {
	...
	private parseBitwiseShift(): EXPRESSION {
		let lhs = this.parseBitwiseNOT();

		while (['>>', '>>>', '<<'].includes(this.current().value)) {
			const op = this.eat().value;
			const rhs = this.parseBitwiseNOT();
			lhs = {
				kind: 'BitwiseExpr',
				lhs,
				rhs,
	...
	private parseBitwiseNOT(): EXPRESSION {
		while (['~'].includes(this.current().value)) {
			return {
				kind: 'BitwiseExpr',
				lhs: this.parseMultiplicativeExpr(),
				// rhs
				op: '~',
			} as BitwiseExpr;
		}
	...
	private parsePrimary(): EXPRESSION {
		...
		switch (tk) {
			...
			case NodeType.OpenParen:
				this.eat();
				const value = this.parseExpr();
				this.expect(NodeType.CloseParen, `Expected ")"`);
